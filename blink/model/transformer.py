...
Recipe D trunk without GAB: token embedding (16 codes) plus a learned square embedding, L pre-RMSNorm
blocks with QK-norm attention (F.scaled_dot_product_attention, no QKV bias, head dim 32) and a
GELU FFN of width 2d, dropout 0, then a final RMSNorm.
"""
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = F.scaled_dot_product_attention(q, k, v.transpose(1, 2))
...
class Trunk(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        x = self.token_embedding(tokens) + self.square_embedding
...
class BlinkNet(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        super().__init__()
        if cfg.gab:
            raise NotImplementedError("GAB-lite is not built in P1; set model.gab = false")
        self.cfg = cfg
