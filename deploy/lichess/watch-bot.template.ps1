...
# Runs `blink lichess watch`: the bot's stop rule every 2 minutes, with no agent session needed.
...
cmd.exe /d /c "C:\dev\blink-chess\.venv\Scripts\blink.exe lichess watch --bot $Bot >> D:\blink\logs\lichess-watch.log 2>&1"
exit $LASTEXITCODE
